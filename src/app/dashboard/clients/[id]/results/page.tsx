...
const GAP_LABELS: Record<string, { dot: StatusColor; label: string; color: string; badge: string }> = {
 ...
 geo_no_aio: { dot: "gray", label: "No AIO Trigger", color: "text-ink-3", badge: "border-line bg-card/5" },
...
export default async function ResultsPage({
 ...
 <span className="text-ink font-bold">Citation Diagnostics</span>
 </div>
 <div className="flex items-center gap-3">
 <h1 className="text-2xl font-heading font-semibold text-ink tracking-tight">AI & Search Results Log</h1>
 ...
 <span className="text-xs font-mono text-ink-3">{count ?? 0} Total Snapshots</span>
 </div>
 </div>

 {/* Track type filter */}
 <div className="flex items-center gap-1.5 bg-[#121215] border border-white/[0.08] p-1.5 rounded-panel shadow-overlay w-fit">
 ...
 : "text-ink-3 hover:text-white hover:bg-card/5"
 }`}
 >
 ...
 <p className="text-lg font-heading font-bold text-ink mb-1">No Diagnostic Results Found</p>
 ...
 className="inline-flex items-center gap-2 rounded-panel bg-surface-2 px-6 py-3 text-xs font-mono font-bold text-black transition-all"
 >
 ...
 {/* Table header */}
 <div className="hidden sm:grid grid-cols-12 gap-2 px-6 py-3.5 bg-black/40 text-xs font-mono font-bold text-gray-400 border-b border-white/[0.06]">
 ...
 {rows.map((r) => {
 const gap = GAP_LABELS[r.gap_label] ?? { dot: "gray" as StatusColor, label: r.gap_label, color: "text-ink-3", badge: "border-line bg-card/5" };
 const tt = TRACK_TYPE_CONFIG[r.track_type as TrackType];
 return (
 <div
 key={r.id}
 className="border-t border-line hover:bg-card/[0.04] transition-all px-6 py-4 text-xs flex flex-col gap-2 sm:grid sm:grid-cols-12 sm:gap-2 sm:items-center"
 >
 {/* Keyword + domain */}
 <div className="sm:col-span-3 min-w-0">
 <p className="text-sm font-heading font-bold text-ink sm:truncate">{r.keyword}</p>
 <p className="text-xs font-mono text-ink-3 truncate mt-0.5">{r.domain}</p>
 </div>
 ...
 <span className={`rounded-md px-2 py-0.5 text-xs font-bold  bg-card/5 border border-line ${tt?.color ?? "text-ink-3"}`}>
 ...
 <div className="sm:col-span-1 sm:text-right font-mono text-ink-3">
 ...
 <span className="text-xs font-mono text-ink-3">Page {page} of {totalPages}</span>
 <div className="flex gap-2">
 {page > 1 && (
 <Link
 href={`/dashboard/clients/${id}/results?page=${page - 1}&track=${trackFilter ?? "all"}`}
 className="inline-flex items-center gap-1 rounded-panel border border-white/10 bg-[#121215] hover:bg-card/[0.06] px-3.5 py-1.5 text-xs font-mono font-bold text-gray-300 transition-all"
 >
