...
export default function AgencyAdminRow({ agency, clients, clientCount, keywordCount }: Props) {
 ...
 return (
 <div className="rounded-panel border border-gray-200 bg-card overflow-hidden">
 {/* Header row */}
 <button
 onClick={() => setOpen((v) => !v)}
 className="w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-gray-50 transition-colors"
 >
 <span className="shrink-0 h-9 w-9 rounded-full bg-attention-soft text-attention flex items-center justify-center font-bold text-sm">
 ...
 {agency.is_pilot ? (
 <span className="rounded-full bg-blue-50 px-2 py-0.5 text-caption font-bold text-blue-700">
 Pilot
 </span>
 ) : (
 <span className="rounded-full bg-attention-soft px-2 py-0.5 text-caption font-bold text-attention">
 Paid
 </span>
 )}
 </div>
 <p className="text-caption text-gray-500 mt-0.5 truncate">{agency.slug}</p>
 </div>
 ...
 <p className="text-caption">Keywords</p>
 </div>
 <div className="text-center min-w-[60px]">
 <p className="text-caption text-gray-500">{new Date(agency.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</p>
 ...
 disabled={saving}
 className={`rounded-md border px-2.5 py-1 text-caption font-semibold transition-colors disabled:opacity-50 ${
 agency.is_disabled
 ...
 onClick={remove}
 disabled={saving}
 className="rounded-md border border-red-300 bg-card px-2.5 py-1 text-caption font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50 transition-colors"
 >
 Delete agency
 </button>
 </div>
 ...
 <p className="text-xs font-bold text-gray-500">Plan</p>
 {editing ? (
 <div className="flex items-center gap-2">
 <button onClick={save} disabled={saving} className="rounded-md bg-ink px-2.5 py-1 text-caption font-semibold text-white hover:bg-ink-2 transition-colors disabled:opacity-50">
 {saving ? "Saving…" : "Save"}
 </button>
 <button onClick={() => { setEditing(false); setErr(null); }} className="text-caption text-gray-500 hover:text-gray-700">Cancel</button>
 </div>
 ) : (
 <button onClick={() => setEditing(true)} className="text-caption text-attention hover:text-attention font-semibold">Edit</button>
 )}
 </div>
 ...
 <span className="block text-caption font-bold text-gray-500 mb-1">Max clients (blank = unlimited)</span>
 <input
 type="number" min={0}
 value={maxClients}
 ...
 <label className="block">
 <span className="block text-caption font-bold text-gray-500 mb-1">Max keywords</span>
 <input
 type="number" min={0}
 value={maxKeywords}
 ...
 <div>
 <p className="text-caption font-bold text-gray-500">Max clients</p>
 <p className="text-sm font-semibold text-gray-900 mt-0.5">{agency.max_clients ?? "Unlimited"}</p>
 </div>
 <div>
 <p className="text-caption font-bold text-gray-500">Max keywords</p>
 <p className="text-sm font-semibold text-gray-900 mt-0.5">{agency.max_keywords}</p>
 </div>
 <div>
 <p className="text-caption font-bold text-gray-500">Account type</p>
 ...
 {/* Clients */}
 <div className="rounded-lg bg-card border border-gray-200 p-3">
 <p className="text-xs font-bold text-gray-500 mb-3">Clients ({clients.length})</p>
 {clients.length === 0 ? (
 ...
 {c.brand_name ?? c.name}
 {!c.brand_name && (
 <span className="ml-1.5 text-caption font-normal text-attention" title="No brand name set - using client name as fallback">⚠ no brand</span>
 )}
 </p>
 <p className={`text-caption truncate ${websiteOk ? "text-gray-500" : "text-red-600 font-medium"}`}>
 {c.website || "— no website —"}
 {!websiteOk && <span className="ml-1.5 text-caption" title="Stored value is not a valid domain. Have the agency re-enter the website to fix citation detection.">⚠ invalid</span>}
 </p>
 </div>
 <div className="shrink-0 flex items-center gap-1.5">
 <span className="text-caption rounded bg-gray-100 text-gray-600 px-1.5 py-0.5 font-bold">{c.service_type}</span>
 <Link
 href={`/admin/clients/${c.id}`}
 className="rounded border border-line bg-attention-soft px-2 py-0.5 text-caption font-bold text-attention hover:bg-attention-soft transition-colors"
 title="Configure engines (super-admin only)"
 >Engines</Link>
 <Link
 href={`/dashboard/clients/${c.id}`}
 className="rounded border border-gray-300 bg-card px-2 py-0.5 text-caption font-bold text-gray-700 hover:bg-gray-50 transition-colors"
 >Open →</Link>
 </div>
 </div>
 );
