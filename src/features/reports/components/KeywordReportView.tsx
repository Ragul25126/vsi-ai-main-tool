...
export default function KeywordReportView({ content }: { content: KeywordReportContent }) {
 ...
 <div className="print-page bg-card rounded-panel border border-gray-200 overflow-hidden">
 ...
 <p className="text-xs font-bold text-red-700 mb-1">AI Mode topic mismatch</p>
...
function ReportHeader({
 ...
 {branding.logoUrl ? (
 <div className="h-16 w-16 rounded-panel bg-card border border-gray-200 p-2 shrink-0 flex items-center justify-center">
 <Image src={branding.logoUrl} alt={branding.displayName} width={56} height={56} className="object-contain max-h-12" unoptimized />
 </div>
 ) : (
 <div className="h-16 w-16 rounded-panel flex items-center justify-center text-white font-bold text-xl shrink-0" style={{ backgroundColor: color }}>
 {branding.displayName.charAt(0).toUpperCase()}
 </div>
 )}
 <div className="min-w-0">
 <p className="text-caption font-bold tracking-[0.18em]" style={{ color }}>{branding.displayName}</p>
 ...
 <p className="text-caption text-gray-500 font-medium">Prepared for</p>
...
function ReportFooter({ branding, color }: { branding: KeywordReportBranding; color: string }) {
 ...
 <div>
 <p className="text-xs text-gray-500 font-semibold">{branding.displayName}</p>
...
function SnapshotStrip({ snapshot }: { snapshot: KeywordReportSnapshot }) {
 ...
 {items.map((it) => (
 <div key={it.label} className="rounded-panel ring-1 ring-gray-200 bg-card p-4">
 <p className="text-caption text-gray-500 font-semibold">{it.label}</p>
 <p className="text-xl font-bold text-gray-900 mt-1">{it.value}</p>
 </div>
 ))}
 </div>
 <p className="text-caption text-gray-400 mt-2">Snapshot captured {shortDate(snapshot.capturedAt)}</p>
 </section>
 );
}
...
function SummaryBody({ narrative }: { narrative: SummaryNarrative }) {
 ...
 <section className="avoid-break">
 <h3 className="text-sm font-bold text-gray-500 mb-3">What we&rsquo;ll do</h3>
 <div className="space-y-2.5">
 {narrative.priorityActions.map((a, i) => (
 <div key={i} className="rounded-panel border border-gray-200 bg-card p-4">
 <div className="flex items-start gap-3">
 <span className="shrink-0 flex h-6 w-6 items-center justify-center rounded-full bg-ink text-xs font-bold text-white">{i + 1}</span>
 ...
 <p className="text-xs font-bold text-attention mb-1">Why this matters</p>
 <p className="text-sm text-gray-800 leading-relaxed">{narrative.whyItMatters}</p>
 </section>

 <section className="avoid-break rounded-panel bg-gray-50 border border-gray-200 p-4">
 <p className="text-xs font-bold text-gray-500 mb-1">Next check-in</p>
...
function DetailedBody({
 ...
 <div key={i} className={`rounded-lg p-3 border ${r.isClient ? "bg-attention-soft border-line" : "bg-gray-50 border-gray-200"}`}>
 <div className="flex items-baseline gap-3">
 <span className="text-xs font-bold text-gray-500 w-6 shrink-0">{r.position}.</span>
 <div className="flex-1 min-w-0">
 <p className="text-sm font-semibold text-gray-900">{r.title}</p>
 <p className="text-caption text-gray-500">{r.domain}{r.isClient ? " · CLIENT" : ""}</p>
 ...
 <span className="text-caption text-gray-500">{snapshot.chatgptMentionCount}× named</span>
 )}
 {snapshot.chatgptCompetitors.length > 0 && (
 <span className="text-caption text-gray-500">vs {snapshot.chatgptCompetitors.length} competitor{snapshot.chatgptCompetitors.length === 1 ? "" : "s"}</span>
 )}
 </div>

 {snapshot.chatgptEntityMatch === false && (
 <div className="mb-3 rounded-lg border border-orange-200 bg-orange-50 p-3">
 <p className="text-xs font-bold text-orange-700 mb-1">Brand-name collision</p>
 ...
 {snapshot.chatgptCompetitors.length > 0 && (
 <div className="mb-3">
 <p className="text-caption text-gray-500 font-semibold mb-1">Competitors ChatGPT named</p>
 ...
 <div key={i} className="rounded-panel border border-gray-200 bg-card p-4">
 <p className="text-xs font-bold text-attention mb-2">{phase.phase}</p>
 ...
 <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-gray-50 text-caption text-gray-500 font-semibold">
 ...
 <div className="col-span-2 text-center font-semibold">{row.rankPosition ? <span className="text-blue-700">#{row.rankPosition}</span> : <span className="text-gray-500 text-caption">Not&nbsp;top&nbsp;10</span>}</div>
...
function TasksBody({ narrative }: { narrative: TasksNarrative }) {
 ...
 return (
 <>
 <div className="rounded-panel border border-blue-200 bg-blue-50 p-4">
 <p className="text-xs font-bold text-blue-700 mb-1">How to use this report</p>
 ...
 <span className={`rounded-full px-2 py-0.5 text-caption font-bold  ${OWNER_COLOR[t.owner]}`}>{t.owner}</span>
 <span className="rounded-full bg-card border border-gray-300 px-2 py-0.5 text-caption font-semibold text-gray-700">{EFFORT_LABEL[t.effort]}</span>
 <span className="rounded-full bg-card border border-gray-300 px-2 py-0.5 text-caption font-semibold text-gray-700">Impact: {t.impact}</span>
 </div>
