...
/** Real setup state for first-use guidance, from the same counts the setup checklists use. */
export async function loadOnboardingState(active: ProjectSummary | null): Promise<OnboardingState> {
  ...
  const [status, competitors] = await Promise.all([loadSetupStatus(active.id), loadProjectCompetitors(active.id)]);
  return {
    hasProject: true,
    known: status.known,
