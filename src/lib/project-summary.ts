...
/**
 * The single aggregation behind Overview, Next Actions, Reports and AI Chat.
...
export async function loadProjectOverview(project: ProjectSummary): Promise<ProjectOverview> {
  const [audit, geo, search, comparisons, tasks] = await Promise.all([
  ...
  if (geo.state === "ok") findings.push(...geoFindings(geo.summary, project.id));
  ...
  return { audit, geo, search, tasks: tasks?.counts ?? null, findings, findingsWithTasks: tasks?.keys ?? [] };
}
